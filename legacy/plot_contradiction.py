...
InputProfile = Literal["standard", "pdf_benchmark"]
INPUT_PROFILE_STANDARD: InputProfile = "standard"
INPUT_PROFILE_PDF_BENCHMARK: InputProfile = "pdf_benchmark"

# Entity labels that PDF extraction often promotes from slugs or action fragments.
PDF_BENCHMARK_ENTITY_STOP_PHRASES: tuple[str, ...] = (
    "ONE OF THE",
    "OF THE WORKMEN",
    "HEAR ",
    "LOOKS AT",
...
PDF_BENCHMARK_ENTITY_HEAD_STOPWORDS: frozenset[str] = frozenset(
    {
        "DAY", "DISSOLVE", "FADE", "HEAR", "INTERCUT", "LOOKING", "NIGHT",
        "POV", "QUICK", "SCREAMING", "SINGING", "LAUGHING", "YEAR-OLD",
    }
)
PDF_BENCHMARK_DESCRIPTOR_ENTITIES: frozenset[str] = frozenset(
    {"box", "eyes", "gauntlet", "wayne"}
)

import spacy
from spacy.language import Language

from legacy.entity_canonicalization import (
    EntityRegistry,
    normalize_name,
...
from legacy.value_normalization import (
    descriptor_axis,
    extract_all_years,
    words_to_int,
)
from legacy.value_normalization import _NUMBER_WORDS as NUMBER_WORDS
...
KNOWLEDGE_YELLOW_SLICKER_PREMATURE_RE: re.Pattern[str] = re.compile(
    r"\b(?:buyer in a )?yellow slicker I can't place\b", re.IGNORECASE
)
KNOWLEDGE_YELLOW_SLICKER_REVEAL_RE: re.Pattern[str] = re.compile(
    r"\bSurveillance:\s*Eddie meets a buyer in a yellow slicker\b|"
    r"\bmeets a buyer in a yellow slicker — not his usual contact\b",
    re.IGNORECASE,
)
KNOWLEDGE_GRAY_SUIT_PARTNER_PREMATURE_RE: re.Pattern[str] = re.compile(
    r"\bgray suit(?:'s|s)\s+partner\b", re.IGNORECASE
)
KNOWLEDGE_PARTNER_REVEAL_RE: re.Pattern[str] = re.compile(
    r"\bI found a partner\.\s*Yours\b", re.IGNORECASE
)
...
# Count entities still extracted and checked in pdf_benchmark mode. Generic
# number+noun pairing (e.g. "204 carrie", "57 ext") is disabled there because
# PDF-extracted action produces spurious chains on clean Hollywood scripts.
PDF_BENCHMARK_COUNT_ENTITIES: frozenset[str] = frozenset(
    {
        "CAST",
        "CLOCK",
        "COURIER",
...
COUNT_PHRASE_PATTERNS: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    (
        re.compile(rf"\b(?P<num>{_COUNT_NUM})\s+families?\b", re.IGNORECASE),
        "FAMILY",
    ),
    (
        re.compile(
            rf"\b(?P<num>{_COUNT_NUM})\s+exchange points?\b", re.IGNORECASE
        ),
        "EXCHANGE",
    ),
)
...
OWNERSHIP_JUNK_OBJECTS: frozenset[str] = frozenset(
    {
        ...
        # Camera / slug / pronoun fragments from PDF extraction
        "POV", "AWAY", "FLAMES", "HIM", "HER", "HE", "SHE", "HERE", "THEM",
        "REPORTERS", "REPORTER", "VOICE", "VOICES", "CAMERA", "SHOT",
    }
)
...
_THREAD_OBJECT_INTO = (
    r"(?:(?:a|an|the|his|her|their)\s+)?"
    r"(?P<object>[A-Za-z][A-Za-z'-]*(?:\s+[A-Za-z][A-Za-z'-]*){0,1})"
    r"(?=\W|$)"
)

OBJECT_REFERENT_THREAD_PATTERNS: tuple[tuple[re.Pattern[str], str, str], ...] = (
    (
        re.compile(
            rf"\b(?:once\s+|is\s+being\s+|being\s+)?"
            rf"(?:sewn|sews|sewing|sew)\s+into\s+{_THREAD_OBJECT_INTO}",
            re.IGNORECASE,
        ),
        "sewn_into",
        "sewn into",
    ...
    (
        re.compile(
            rf"\b(?:once\s+)?(?:wrapped|wraps|wrapping|wrap)\s+"
            rf"(?:\w+\s+){{0,4}}in\s+{_THREAD_OBJECT_INTO}",
            re.IGNORECASE,
        ),
        "wrapped_in",
        "wrapped in",
    ...
    (
        re.compile(
            rf"\b(?:once\s+)?(?:placed|places|placing|place|put|puts|putting)\s+"
            rf"(?:\w+\s+){{0,4}}in(?:to)?\s+{_THREAD_OBJECT_INTO}",
            re.IGNORECASE,
        ),
        "placed_in",
        "placed in",
    ...
    (
        re.compile(
            rf"\b(?:once\s+)?(?:shoved|shoves|shoving|shove)\s+"
            rf"(?:\w+\s+){{0,4}}into\s+{_THREAD_OBJECT_INTO}",
            re.IGNORECASE,
        ),
        "shoved_into",
        "shoved into",
...
THREAD_OBJECT_STOPWORDS: frozenset[str] = frozenset(
    {
        "a",
        "an",
        "and",
...
REFERENT_SWAP_SYNONYM_GROUPS: tuple[frozenset[str], ...] = (
    frozenset({"blanket", "cloth", "sheet", "shroud", "towel", "wrap"}),
    frozenset({"envelope", "folder", "packet", "pouch"}),
    frozenset({"bag", "bundle", "sack", "tarp"}),
)


def _thread_object_head(phrase: str) -> str:
    """Return the head noun from an action-thread object phrase."""
    words = re.findall(r"[a-z]+", phrase.lower())
    filtered = [word for word in words if word not in THREAD_OBJECT_STOPWORDS]
    if not filtered:
        return ""
    return filtered[0]


def _referent_swap_pair(head_a: str, head_b: str) -> bool:
    """Return True when two noun heads likely denote the same referent slot."""
    if not head_a or not head_b or head_a == head_b:
        return False
    for group in REFERENT_SWAP_SYNONYM_GROUPS:
        if head_a in group and head_b in group:
            return True
    return False


def _extract_object_referent_threads(
    action_text: str,
) -> list[tuple[str, str, str]]:
    """Extract ordered action-thread mentions from scene action text.

    Args:
        action_text: Joined action lines for one scene.

    Returns:
        Ordered list of (frame_key, object_head, matched_excerpt).
    """
    hits: list[tuple[int, str, str, str]] = []
    cursor = 0
    for line in action_text.splitlines():
        for pattern, frame_key, _label in OBJECT_REFERENT_THREAD_PATTERNS:
            for match in pattern.finditer(line):
                head = _thread_object_head(match.group("object"))
                if len(head) < 3:
                    continue
                hits.append((cursor + match.start(), frame_key, head, match.group(0)))
        cursor += len(line) + 1
    hits.sort(key=lambda item: item[0])
    return [(frame_key, head, excerpt) for _, frame_key, head, excerpt in hits]


def _scene_body_text(scene: SceneBlock) -> str:
    """Return scene body text (excluding the heading) for action-thread scans."""
    lines = scene.raw_text.splitlines()
    if len(lines) <= 1:
        return ""
...
def _is_pdf_benchmark_junk_entity(entity: str) -> bool:
    """Return True when a PDF-extracted entity label is slug or action noise.

    Used in ``pdf_benchmark`` mode to suppress trait, age, and ownership false
    positives on Hollywood scripts without changing writer-corpus detection.
    """
    ...
    if normalized.endswith("-") or normalized.endswith(" -"):
        return True
    if "(CONT'D)" in normalized or "CONT'D" in normalized:
        return True
    words = normalized.split()
    if len(words) > 4:
        return True
    for phrase in PDF_BENCHMARK_ENTITY_STOP_PHRASES:
        if phrase in normalized:
            return True
    if words[0] in PDF_BENCHMARK_ENTITY_HEAD_STOPWORDS:
        return True
...
def _extract_knowledge_claims(scenes: list[SceneBlock]) -> list[_KnowledgeClaim]:
    ...
    for scene in sorted(scenes, key=lambda item: item.scene_number):
        for kind, text in iter_scene_lines(scene):
            ...
            if KNOWLEDGE_YELLOW_SLICKER_PREMATURE_RE.search(text):
                claims.append(
                    _KnowledgeClaim(
                        scene=scene,
                        entity="YELLOW SLICKER BUYER",
                        claim="premature",
                        excerpt=text.strip(),
                    )
                )
            if KNOWLEDGE_YELLOW_SLICKER_REVEAL_RE.search(text):
                claims.append(
                    _KnowledgeClaim(
                        scene=scene,
                        entity="YELLOW SLICKER BUYER",
            ...
            if KNOWLEDGE_GRAY_SUIT_PARTNER_PREMATURE_RE.search(text):
                claims.append(
                    _KnowledgeClaim(
                        scene=scene,
            ...
            if KNOWLEDGE_PARTNER_REVEAL_RE.search(text):
                claims.append(
                    _KnowledgeClaim(
                        scene=scene,
                        entity="CLIENT PARTNER",
...
class ContradictionEngine:
    ...
    def __init__(
        ...
        self.input_profile: InputProfile = input_profile
        self._fact_counter: int = 0

    def _uses_pdf_benchmark_numeric_rules(self) -> bool:
        """Return True when numeric_count should use PDF/benchmark guardrails."""
        return self.input_profile == INPUT_PROFILE_PDF_BENCHMARK

    def _uses_pdf_benchmark_profile(self) -> bool:
        """Return True when PDF/benchmark guardrails apply beyond numeric_count."""
        return self.input_profile == INPUT_PROFILE_PDF_BENCHMARK
    ...
    def _extract_count_facts(self, scene: SceneBlock, store: FactStore) -> None:
        ...
        for line in action_lines + dialogue_lines:
            ...
            if self._uses_pdf_benchmark_numeric_rules():
                continue
    ...
    def _add_object_state_facts(
        ...
        for pattern, _shape in patterns:
            ...
            if len(object_key) < 2:
                continue
            if object_key in OWNERSHIP_JUNK_OBJECTS:
                continue
    ...
    def run_tier1(
        ...
        contradictions.extend(self._check_object_referent_swap(scenes))
    ...
    def _check_character_knowledge(
        ...
        glass-key timing, ``THEY KNOW`` dialogue, signal naming, railroad-feud
        reveals, yellow-slicker buyer timing, and gray-suit partner reveals).
        """
        ...
        yellow_slicker = by_entity.get("YELLOW SLICKER BUYER", {})
        slicker_premature = yellow_slicker.get("premature")
        slicker_reveal = yellow_slicker.get("reveal")
        if slicker_premature is not None and slicker_reveal is not None:
            _emit_pair(
                slicker_premature,
                slicker_reveal,
                "YELLOW SLICKER BUYER",
                (
                    "The yellow-slicker buyer is referenced before it can be "
                    f"placed in {slicker_premature.scene.scene_id}, but "
                    f"{slicker_reveal.scene.scene_id} is the first surveillance "
                    "sightings of that contact."
                ),
            )

        client_partner = by_entity.get("CLIENT PARTNER", {})
        partner_premature = client_partner.get("premature")
        partner_reveal = client_partner.get("reveal")
        if partner_premature is not None and partner_reveal is not None:
            _emit_pair(
                partner_premature,
                partner_reveal,
                "CLIENT PARTNER",
                (
                    "The client is named as the gray suit's partner in "
                    f"{partner_premature.scene.scene_id} before "
                    f"{partner_reveal.scene.scene_id} confirms it with "
                    "''I found a partner. Yours.''"
                ),
            )
        return results
    ...
    def _check_name_consistency(
        ...
        for name_a, name_b, distance in registry.near_duplicate_pairs(
            ...
            if not _name_drift_plausible_pair(name_a, name_b):
                continue
            if self._uses_pdf_benchmark_profile() and (
                name_a.rstrip().endswith("-") or name_b.rstrip().endswith("-")
            ):
                continue
    ...
    def _check_numeric_count(
        ...
        for noun, facts in by_noun.items():
            if (
                self._uses_pdf_benchmark_numeric_rules()
                and noun not in PDF_BENCHMARK_COUNT_ENTITIES
            ):
                continue
            ordered = sorted(facts, key=lambda item: item.scene_number)
            if not ordered:
    ...
    def _check_age_conflict(
        ...
        for entity, facts in by_entity.items():
            if self._uses_pdf_benchmark_profile() and _is_pdf_benchmark_junk_entity(
                entity
            ):
                continue
            for left_index in range(len(facts)):
                for right_index in range(left_index + 1, len(facts)):
                    ...
                    try:
                        earlier_age = int(str(earlier.value))
                        later_age = int(str(later.value))
                    except ValueError:
                        continue
                    if self._uses_pdf_benchmark_profile() and later_age > earlier_age:
                        continue
                    if self._uses_pdf_benchmark_profile() and (
                        earlier_age <= 3 or later_age <= 3
                    ):
                        continue
                    scene_gap = later.scene_number - earlier.scene_number
                    if self._uses_pdf_benchmark_profile() and scene_gap >= 20:
                        continue
                    results.append(
                        Contradiction(
                            contradiction_id=_new_contradiction_id(),
    ...
    def _check_object_identity(
        ...
        for entity, facts in by_entity.items():
            if self._uses_pdf_benchmark_profile() and (
                entity.lower() in NON_PROP_OBJECTS
                or entity.lower() in PDF_BENCHMARK_DESCRIPTOR_ENTITIES
                or _is_pdf_benchmark_junk_entity(entity)
            ):
                continue
            ordered = sorted(facts, key=lambda item: item.scene_number)
    ...
    def _check_object_referent_swap(
        self, scenes: list[SceneBlock]
    ) -> list[Contradiction]:
        """Flag same-scene action threads that swap the head noun for one referent.

        Detects patterns such as ``wrapped in a blanket`` followed by
        ``Once wrapped in the towel`` within the same scene — the verb frame
        anchors the referent slot; conflicting head nouns in the same class
        (blanket/towel, envelope/folder) indicate a writer slip.
        """
        results: list[Contradiction] = []
        for scene in scenes:
            body_text = _scene_body_text(scene)
            mentions = _extract_object_referent_threads(body_text)
            by_frame: dict[str, list[tuple[str, str]]] = {}
            for frame_key, head, excerpt in mentions:
                by_frame.setdefault(frame_key, []).append((head, excerpt))

            for frame_key, entries in by_frame.items():
                heads_in_order: list[str] = []
                excerpts_by_head: dict[str, str] = {}
                for head, excerpt in entries:
                    if head in excerpts_by_head:
                        continue
                    excerpts_by_head[head] = excerpt
                    heads_in_order.append(head)

                emitted = False
                for index, head in enumerate(heads_in_order):
                    if emitted:
                        break
                    for prior_head in heads_in_order[:index]:
                        if not _referent_swap_pair(prior_head, head):
                            continue
                        frame_label = next(
                            label
                            for _pattern, key, label in OBJECT_REFERENT_THREAD_PATTERNS
                            if key == frame_key
                        )
                        results.append(
                            Contradiction(
                                contradiction_id=_new_contradiction_id(),
                                scene_id_a=scene.scene_id,
                                scene_id_b=scene.scene_id,
                                scene_number_a=scene.scene_number,
                                scene_number_b=scene.scene_number,
                                fact_a=None,
                                excerpt_b=excerpts_by_head[head],
                                contradiction_type="object_referent_swap",
                                explanation=(
                                    f"In {scene.scene_id}, the same {frame_label} "
                                    f"action thread names both '{prior_head}' and "
                                    f"'{head}' — likely the same referent with "
                                    "conflicting nouns."
                                ),
                                confidence=0.68,
                                tier=1,
    ...
    def run_tier2(
        ...
        for entity, facts in facts_by_entity.items():
            if len(facts) < 2:
                continue
            if _normalize_token(entity) in OWNERSHIP_JUNK_OBJECTS:
                continue
            if self._uses_pdf_benchmark_profile() and _is_pdf_benchmark_junk_entity(
                entity
            ):
    ...
    def _tier1_scene_entity_coverage(
        ...
        for contradiction in tier1_results:
            ...
            if contradiction.fact_a is None:
                covered.add(
                    (
                        scene_ids[0],
                        scene_ids[1],
