...
| **D6** — fact + causal dependency edges | ``fact`` edges link scenes that establish plot state to later scenes referencing the same entity; ``causal`` edges link explicit backward dialogue to the most recent prior scene sharing a speaker. Both use reserved ``EDGE_WEIGHTS``; open-domain "what" resolution deferred to C4. | _pending_ |
