...
| **Caveat 2 / D3** — Signal 3 grammatical-role detection | A caps name parsed as the `nsubj`/`nsubjpass` of an **animate-only verb** (communication, expression, gesture, cognition — `AGENTIVE_PERSON_VERB_LEMMAS`) is now promoted to a character and excluded from props, recovering invented names with no cue, title, fact phrasing, or NER hit ("KORREK whispers"). Motion/machine verbs are excluded so action props ("the DELOREAN races", "the PHONE rings") are not promoted; detection runs on both raw and title-cased parses and requires the name to appear as an ALL-CAPS span. | `c05ee68` |
| **D4** — first-seen-only edges | Reuse edges now fan out from *every* prior scene that featured an item (not just the first), additively. Deleting an intermediate carrier scene now surfaces the downstream scenes whose dependency flows through it; the first-occurrence edge and the orphan set are unchanged. | _pending_ |
| **D6** — fact + causal dependency edges | ``fact`` edges link scenes that establish plot state to later scenes referencing the same entity; ``causal`` edges link explicit backward dialogue to the most recent prior scene sharing a speaker. Both use reserved ``EDGE_WEIGHTS``; open-domain "what" resolution deferred to C4. | `02ea523` |
| **D7 / X3** — shared spaCy + single parse per scene | ``nlp_shared.get_shared_nlp()`` singleton; engines accept injected ``nlp``; ``_parse_action_docs`` shares raw + title-cased docs across NER and agentive detection. | _pending_ |
| **D8** — sub-location headings | ``_extract_locations_from_heading`` returns primary + sub-location hierarchy, stripping time-of-day suffixes. | _pending_ |
...
| D4 | ~~**First-seen-only edges.** Reuse links back to the *first* scene, not each prior scene. Deleting an intermediate scene won't show downstream breakage that conceptually flows through it.~~ **RESOLVED** — a reuse now links to *every* prior scene that featured the item (additively: the original first-occurrence edge is kept, intermediate edges labelled "also appears" are added), so `get_delete_impact` ripples through intermediate carrier scenes (see §3). Orphan set is unchanged. | `get_delete_impact` now reflects ripple effects on chained setups. | ~~Medium~~ Resolved |
...
| D6 | ~~**`fact` and `causal` dependency edges are unimplemented**~~ **RESOLVED (v1)** — ``fact`` edges link establishing plot facts to later entity references; ``causal`` edges detect explicit backward dialogue phrasing and link to the most recent prior scene sharing a speaker. Open-domain causal resolution ("what you did" → which scene) still needs coreference (C4). | Story-state and explicit dialogue dependencies now appear in the graph and delete-impact. | ~~Medium~~ Low |
| D7 | ~~**Performance: up to 2 spaCy passes per scene** (`nlp(action_text)` + a second pass on title-cased text), and the model is loaded **separately in each engine**.~~ **RESOLVED** — ``get_shared_nlp()`` loads the model once per process; both engines accept an injected pipeline. ``_parse_action_docs`` parses raw + title-cased action text once per scene and shares docs across NER and agentive detection (was up to 3 title-case parses). | Lower NLP cost and memory; SLA risk reduced. | ~~Medium~~ Resolved |
| D8 | ~~**Locations are single-level** (primary place only; time-of-day and sub-locations dropped).~~ **RESOLVED** — ``_extract_locations_from_heading`` returns a hierarchy (e.g. ``["HOUSE", "HOUSE KITCHEN"]``), stripping trailing time-of-day segments. Sub-locations stay distinct; primary location still links scenes. | Kitchen vs bedroom no longer collapse to one key. | ~~Low~~ Resolved |
...
13. ~~**Single shared spaCy load (D7/X3)**~~ **DONE** — ``nlp_shared.get_shared_nlp()``; injected pipeline; ``_parse_action_docs`` deduplicates per-scene NLP.
14. ~~**Implement fact/causal dependency edges (D6)**~~ **DONE (v1).** ``fact`` + ``causal`` edge types wired into ``build_graph``; analyser passes a shared ``FactStore`` to avoid double extraction.
